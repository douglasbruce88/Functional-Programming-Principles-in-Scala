...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    if (p(elem)) leftright.incl(elem) else leftright
  }
  ...
  def mostRetweeted: Tweet = {
    ...
    else if(mostleft.retweets > mostright.retweets) mostleft
    else mostright
  }
...
object GoogleVsApple {
  ...
  def filterTweet(word: String, tweets: TweetSet): TweetSet =
    tweets.filter(t => t.text.contains(word))

  def filterTweets(words: List[String], tweets: TweetSet) =
    tweets.filter(t => words.exists(w => t.text.contains(w)))
